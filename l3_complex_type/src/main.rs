...
impl School {
    ...
    pub fn grades(&self) -> Vec<u32> {
        let mut _grades: Vec<u32> = self.students.values().cloned().collect();
        _grades.sort();
        _grades.dedup();
        _grades
    }
    ...
    pub fn grade(&self, grade: u32) -> Vec<String> {
        let _students = &self.students;
        let mut _result = Vec::<String>::new();

        for (_s, _g) in _students {
            if *_g == grade {
                _result.push(_s.to_string());
            }
        }
...
fn main() {
    ...
    data.insert(String::from("Alice"), 9);
    data.insert(String::from("Bob"), 8);
    data.insert(String::from("Cevil"), 9);
    ...
    println!("\nDanh sach student: {:?}", school.students);
    println!("Danh sach diem khong trung: {:?}", school.grades());
    println!("Danh sach student 9 diem: {:?}", school.grade(9));
